...
def evaluate(
    ...
    device,
    prompt_length=50,
    ps="0.85,0.87,0.89,0.91,0.93,0.95,0.97,0.99",
):
    logger.info("***** Running Generation *****")
    logger.info("  Num examples = %d", len(eval_dataset))
    logger.info("  Batch size = %d", eval_batch_size)
    human_gens, topp_gens = [], {}
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            for p in ps:
                ...
                topp_gens[str(p)].extend(
                    tokenizer.batch_decode(out_ids, skip_special_tokens=True)
                )
    data = {"human": human_gens}
    for p in topp_gens:
        assert len(human_gens) == len(topp_gens[p])
        data[f"topp(p={p})"] = topp_gens[p]
